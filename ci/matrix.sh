...
export BUILDER_IMAGE_NAME="getrektcoin-builder-$BUILD_TARGET-$JOB_NUMBER"
...
if [ "$BUILD_TARGET" = "arm-linux" ]; then
  ...
  export USE_SHELL="/bin/getrektcoin"
  export PYZMQ=true
  export RUN_TESTS=true
  ...
  export CPPFLAGS="-DDEBUG_LOCKORDER -DENABLE_GRC_DEBUG"
  export PYZMQ=true
  export RUN_TESTS=true
