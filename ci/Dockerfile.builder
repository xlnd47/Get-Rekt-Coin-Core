...
# getrektcoin_hash
RUN git clone https://github.com/getrektcoinpay/getrektcoin_hash
RUN cd getrektcoin_hash && python3 setup.py install
...
RUN groupadd -g ${GROUP_ID} getrektcoin
RUN useradd -u ${USER_ID} -g getrektcoin -s /bin/bash -m -d /getrektcoin getrektcoin
...
RUN mkdir /getrektcoin-src && \
  mkdir -p /cache/ccache && \
  mkdir /cache/depends && \
  mkdir /cache/sdk-sources && \
  chown $USER_ID:$GROUP_ID /getrektcoin-src && \
  chown $USER_ID:$GROUP_ID /cache && \
  chown $USER_ID:$GROUP_ID /cache -R
WORKDIR /getrektcoin-src
