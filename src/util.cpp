...
const char * const BITCOIN_CONF_FILENAME = "getrektcoin.conf";
const char * const BITCOIN_PID_FILENAME = "getrektcoind.pid";
    ...
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL || *cacheCounter != logAcceptCategoryCacheCounter.load())
        {
            ...
            if (mapMultiArgs.count("-debug")) {
                ...
                // "getrektcoin" is a composite category enabling all GetRektCoin-related debug output
                if(ptrCategory->count(std::string("getrektcoin"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\GetRektCoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\GetRektCoin
    // Mac: ~/Library/Application Support/GetRektCoin
    // Unix: ~/.getrektcoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "GetRektCoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/GetRektCoin";
#else
    // Unix
    return pathRet / ".getrektcoin";
#endif
#endif
}
...
void ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good()){
        // Create empty getrektcoin.conf if it does not excist
...

    {
        ...
        {
            // Don't overwrite existing settings so command line settings override getrektcoin.conf
