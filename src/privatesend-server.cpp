...
void CPrivateSendServer::ProcessMessage(CNode* pfrom, const std::string& strCommand, CDataStream& vRecv, CConnman& connman)
{
    ...
    if (fLiteMode) return; // ignore all GetRektCoin related functionality
...
/*
    ...
    allow endless transaction that would bloat GetRektCoin and make it unusable. To
...
{
    if (fLiteMode) return;        // disable all GetRektCoin specific functionality
