...
// which are part of the GetRektCoin repo will allow modification to the Jenkinsfile.
...
for(int i = 0; i < targets.size(); i++) {
  ...
  tasks["${target}"] = {
    node {
      ...
      withEnv(env) {
        def builderImageName="getrektcoin-builder-${target}"
        ...
        builderImage.inside("-t") {
          ...
          sh "cp -ra ${pwd}/. /getrektcoin-src/"

          // restore cache
          def hasCache = false
          try {
            copyArtifacts(projectName: "getrektcoinpay-getrektcoin/${BRANCH_NAME}", optional: true, selector: lastSuccessful(), filter: "ci-cache-${target}.tar.gz")
          } catch (Exception e) {
          }
          if (fileExists("ci-cache-${target}.tar.gz")) {
            hasCache = true
            echo "Using cache from getrektcoinpay-getrektcoin/${BRANCH_NAME}"
          } else {
            try {
              copyArtifacts(projectName: 'getrektcoinpay-getrektcoin/develop', optional: true, selector: lastSuccessful(), filter: "ci-cache-${target}.tar.gz");
            } catch (Exception e) {
            }
            if (fileExists("ci-cache-${target}.tar.gz")) {
              hasCache = true
              echo "Using cache from getrektcoinpay-getrektcoin/develop"
            }
          }

          if (hasCache) {
            sh "cd /getrektcoin-src && tar xzf ${pwd}/ci-cache-${target}.tar.gz"
          } else {
            sh "mkdir -p /getrektcoin-src/ci-cache-${target}"
          }

          stage("${target}/depends") {
            sh 'cd /getrektcoin-src && ./ci/build_depends.sh'
          }
          stage("${target}/build") {
            sh 'cd /getrektcoin-src && ./ci/build_src.sh'
          }
          stage("${target}/test") {
            sh 'cd /getrektcoin-src && ./ci/test_unittests.sh'
          }
          stage("${target}/test") {
            sh 'cd /getrektcoin-src && ./ci/test_integrationtests.sh'
          }

          // archive cache and copy it into the jenkins workspace
          sh "cd /getrektcoin-src && tar czfv ci-cache-${target}.tar.gz ci-cache-${target} && cp ci-cache-${target}.tar.gz ${pwd}/"
        }
